                     ...
                     param_struct const* const params_arg /*= nullptr*/ )
{
    ...
    if (params.report_epoch > 0)
    {
        objective_val.conservativeResize(obj_idx + 1000 + ((nProj-projection_dim) * params.max_iter / params.report_epoch));
    }

    if (params.report_avg_epoch > 0)
    {
        objective_val.conservativeResize(obj_idx_avg + 1000 + ((nProj-projection_dim) * params.max_iter / params.report_avg_epoch));
    }
    ...
    for(; projection_dim < nProj; projection_dim++)
    {
        ...
        // initialize w as vector between the means of two random classes.
        // should find clevered initialization schemes
        int c1 = ((int) rand()) % nClass;
        int c2 = ((int) rand()) % nClass;
        if (c1 == c2)
        {
            c2=(c1+1)%nClass;
        }
        ...
        print_report<EIGENTYPE>(projection_dim,batch_size, nClass,C1,C2,lambda,w.size(),x);
