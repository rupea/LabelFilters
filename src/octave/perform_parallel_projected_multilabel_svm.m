function [out_final, out_final_tr] = perform_parallel_projected_multilabel_svm(exp_name, C, option, noClasses, exp_dir, force_retrain, projection_file = [], projection_dir = [], project_str = [])
  ...
  if (retrain)
    if noClasses < 10,
      ...
      batch_length = floor(noClasses / 1000);  % find the interval of the batch sizes
    ...
    file_expr = @(idx) [exp_name "_" num2str(C) "_" num2str(label_range(idx)) "_" num2str(label_range(idx+1)-1) "_projected_" project_str];    
    
    cur_file = @(idx) ["svm_results/svm_" file_expr(idx) ".mat"];    
    
    octave_cmd = @(idx) sprintf("octave -q --path %s --eval \"perform_projected_multilabel_svm_batch('%s','%s', '%s', '%s', '%s' ,%f,%d,%d)\"", ...
    path(), exp_name,exp_dir,projection_file, projection_dir, project_str, C, label_range(idx),label_range(idx+1)-1);
