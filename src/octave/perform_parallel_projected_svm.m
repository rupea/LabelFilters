function [out_final] = perform_parallel_projected_svm(exp_name, C, option, exp_dir, projection_file, projection_dir, project_str,  force_retrain, tr_label)
...
if exist(filename,"file") && ~force_retrain
    ...
    file_expr = @(idx) [exp_name "_" num2str(C) "_" num2str(label_range(idx)) "_" num2str(label_range(idx+1)) "_projected_" project_str];    
    
    cur_file = @(idx) ["svm_results/svm_" file_expr(idx) ".mat"];    
    
    octave_cmd = @(idx) sprintf("octave -q --path %s --eval \"perform_projected_svm_batch('%s','%s', '%s', '%s', '%s' ,%f,%d,%d)\"", ...
    path(), exp_name,exp_dir,projection_file, projection_dir, project_str, C, label_range(idx),label_range(idx+1));
